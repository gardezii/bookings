...
def TurnlightOffTask(ifttt_key, slot_id):
	try:
		...
		saveSchedulerHistory(ifttt_key=ifttt_key.name, job_type="off_event", slot_id=slot_id, status="success")
	except Exception as e: 
		print ("Something went wrong while running the off event scheduler")
		saveSchedulerHistory(ifttt_key=ifttt_key.name, job_type="off_event", slot_id=slot_id, status="fail")
...
def TurnlightOnTask(ifttt_key, slot_id):
	try: 
		...
		saveSchedulerHistory(ifttt_key=ifttt_key.name, job_type="on_event", slot_id=slot_id, status="success")
	except Exception as e:
		print ("Something went wrong while running the off event scheduler")
		saveSchedulerHistory(ifttt_key=ifttt_key.name, job_type="on_event", slot_id=slot_id, status="fail")
